...
为了让 GitHub record 更清晰：

- 根目录 README 已更新为当前最终版入口。
- 旧操作文档已移动到 `docs/history/`。
- 旧启动脚本已移动到 `archive/legacy_scripts/`。
- 实机 debug 脚本已移动到 `tools/hardware_debug/`。
- `logs/*.log` 已从 Git 跟踪中移除，`.gitignore` 已加入 `logs/`。
...
不推荐把整个 `r1_arm_control` 改成 `staff_gripper_control`，因为它还负责 KFS elevator 和 KFS horizontal。当前系统已经完结，最稳妥的做法是先只清理文档和旧脚本，不大规模改 ROS package 名。
