...
def test_arm_safe_state_matches_relays_two_to_seven():
    assert DEFAULT_ARM_SAFE_STATE == [0, 1, 0, 1, 1, 0]
...
def test_selected_inclination_indices():
    assert PneumaticGripperJoystickBridgeNode.selected_inclination_index(7) == 5
    assert PneumaticGripperJoystickBridgeNode.selected_inclination_index(8) == 2
